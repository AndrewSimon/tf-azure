...
## Trouble-shooting
   ...
   8. VM is running but not registering as a github runner: anything to do with registering or de-registering azure resources to/for the VM is via the user-data section of the embedded python code. Assign a public IP to the VM NIC, and connect to your VM, which is azureuser@some-ip and password is whatever you set with adminpass=<admin-password>.  The /var/lib/cloud/instance/user-data.txt file is what is executed at system launch, and the log goes to /var/log/cloud-init-output.log.  Inspect if run.sh was executed either via cloud-init-output.log and/or via "ps -ef|grep run.sh" on the command line. If run.sh was not run, then check the user-data.txt file itself. This file should contain the code embedded in the USERDATA portion of your python script.  If there is code, you can run it with "bash user-data.txt".  If no code or it is corrupted (often, the file contains the encrytped code, not the decrypted code needed at startup), the function is corrupted in the Function App. Uploading the function alone (again) without deleting the Function App itself may not fix it.  Delete the Function App and the function_app.py file and re-run terraform apply.  Ensure the function uploads again.
   9.  Return Code 429 from Github webhook response. The custom message: <b>UserData did not decode.  Azure needs a minute or two before providing another useful VM.</b> is provided via python definition I wrote to catch when it happens.  This is some sort of caching bug in Azure Function launch affecting decoding of the encoded VM user-data. It takes about 2 minutes for the Azure Function to be 'reset' on it's own sufficienty to be able to cleanly create a new VM. 
