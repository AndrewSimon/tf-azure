...
<i>Error in function call on azure_function.tf, Call to function "filemd5" failed: <B>function returned an inconsistent result</B></i>:
   ...
   7. If you successfully installed Function App Core Tools, you will be able to run <i>func start</i> in the tf-azure directory and start a local Function App! You can post data with curl, fiddler or other client to <i>http://localhost:7071/api/launch_vm</i> and output not directed to the client will come into the screen running <i>func start</i> as standard error and standard out.  While trouble-shooting, you update the function_app.py code in an editor window, and saved code changes will automatically reload into the <i>func start</i> run, you do not need to restart it.  You can then re-run the client, hit the localhost api endpoint, and see if your code edits changed/fixed things.  Repeat as needed.  When done editing function_app.py <b>remember to update function_app.tf</b> as all of your function_app.py updates will be overwritten next time you run terraform apply!
   8. If your function uploads but gives error 500, it means your function did kind of run!   But likely problem with python code, empty values (did you export your variables?) and so on can also be diagnosed via Function App monitoring via your Insights Dashboard, also created for you by this terraform plan.  By drilling down on any summaries that show errors, you can get details on those errors.  They are usually 'pretty easy' to fix, either in the terraform code, or in the embedded python, depending on the issue.  For instance, permission or storage issues are usually fixed via resources, roles and profiles in terraform code.  W
   9. VM is running but not registering as a github runner: anything to do with registering or de-registering azure resources to/for the VM is via the user-data section of the embedded python code.  The user-data is what is run at system launch, and the log goes to /var/log/cloud-init-output.log.  Use the public IP and connect to your device, which is azureuser@some-ip and password is whatever you set with adminpass=<admin-password> to inspect for errors. Or, once logged in, you can try to connect to the github server manually using instructions provided via the Github runner UI (Actions-->Runners) menu option to get the command line option (including a new token) to run.  Do this to see what errors come up: they are probably the same as the ones found in /var/log/cloud-init-output.log.m Fix github actions self-hosted registration and life-cycle changes via the user-data in the embedded python in azure_function.tf    

## Costs

A <i>terraform apply</i> usually triggers the webhook and function as it's (re)deployed, whether you git push to trigger the function or not, and will launch a virtual machine!  It will also join your github repo as a self-hosted actions runner and start running jobs tagged as 'self-hosted'!  Fortunately, you must run the terraform plan two or three times from the first run before your first actions runner joins and can work. All other tiggers will work because the app route is up and running. With this webhook trigger configured to your own repo(s), you will launch every time you a) send an authorized payload (via git push) to any of those repos configured, b) manually trigger the webhook via 'Redelivery' option in the Github UI, or c) re-deploy a webhook via terraform apply. That could be quite a few launches if you are pushing back to your own git repo regularly and frequently, and/or updating or manually triggering the webhook(s), so beware! 

This plan <i>tf-azure</i> has a working full life-cycle, meaning it successfully joins the GH server becoming a functional Github self-hosted actions runner, that also knows how and when to safely kill itself, and remove vm storage.  Not yet implement but in progress is removing public ip in all situations along with vm's associated nic.  As of this writing, the tf-azure plan brings up a generic 'latest' ubutu (lts) via webhook trigger and joins to your github server as a self-hosted actions runner!  

Most importantly, this plan is an easy and convenient way to deploy the infrastructure to run VM runners in their own resource group, dedicated virtual network, security group, RBAC role, and so on, whether you need a github self-hosted actions runner, or not! 
